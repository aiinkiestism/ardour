...
void
PluginManager::cancel_plugin_scan_timeout ()
{
	_cancel_scan_timeout = true;
}

void
PluginManager::cancel_plugin_all_scan_timeout ()
{
	_cancel_all_scan_timeout = true;
}
	...
	for (i = 0; ; ++i) {
		...
		if (!ladspa_plugin_whitelist.empty()) {
			if (find (ladspa_plugin_whitelist.begin(), ladspa_plugin_whitelist.end(), descriptor->UniqueID) == ladspa_plugin_whitelist.end()) {
				psle->msg (PluginScanLogEntry::OK, string_compose(_("LADSPA ignored blacklisted unique-id %1."), descriptor->UniqueID)); // XXX
				continue;
			}
		}
		...
		if (!found) {
			...
			psle->msg (PluginScanLogEntry::OK, string_compose(_("LADSPA ignored plugin with dupliucate id %1."), descriptor->UniqueID)); // XXX
		}
...
PluginManager::auv2_discover (AUv2DescStr const& d, bool cache_only)
{
	...
	if (!cache_only && auv2_scanner_bin_path.empty ()) {
#if 0
		/* TODO: direct scan in the host's process */
		psle->reset ();
		psle->msg (PluginScanLogEntry::TimeOut, _("AU scanner application is missing."));
		return -1;
#else
		if (cache_file.empty ()) {
	...
	for (XMLNodeConstIterator i = tree.root()->children().begin(); i != tree.root()->children().end(); ++i) {
		try {
			// TODO use callback like scan_and_cache
			AUv2Info nfo (**i);
...
PluginManager::vst2_discover (string path, ARDOUR::PluginType type, bool cache_only)
{
	...
	if (!cache_only && vst2_scanner_bin_path.empty ()) {
		/* TODO: direct scan in the host's process */
		psle->reset ();
		psle->msg (PluginScanLogEntry::TimeOut, _("VST2 scanner application is missing.")); // XXX
		return -1;
	}
